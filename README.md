...
Capture packets in different environment configurations and compare them to determine if `charlie` protects network traffic per the claim made in the _[Research aim](#research-aim)_ section.
...
Some effort was required to setup the environment such that IP packets could be monitored. This was likely due to VMWare's USB driver being unable to keep up with high speed 802.11 protocols. In order to mitigate this the device being monitored, `RabbitSeason`, needed to be dropped to 802.11b.

There is a significant amount of work that can, and should, be done to further audit the Mudi device in order to validate the vendor's claims and the devices many features, e.g.:
